...
// the addValidationErrors() signature here doesn't match the actual function in ApiError.java
...
//    }
	...
	public ResponseEntity<Object> handleConstraintViolation(jakarta.validation.ConstraintViolationException ex,
		...
		ApiError apiError = new ApiError(BAD_REQUEST);
		apiError.setMessage("Validation error constraint");
		apiError.addValidationErrors(ex.getConstraintViolations());
		return buildResponseEntity(apiError);
	}
	 ...
	 */
// formal parameter is of a class not found, resolving ended up matching an existing class
